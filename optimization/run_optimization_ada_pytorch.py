...
from models.stylegan2adapytorch import legacy;
...
def main(args):
    ...
    # start from a particular latent vector  
    if args.latent_path:
        latent_code_init = torch.load(args.latent_path).cuda()
    else: # start from a random latent vector
        latent_code_init = torch.from_numpy(np.random.randn(1, G.z_dim)).to(device)

    # only supports unconditional network
    label = torch.zeros([1, G.c_dim], device=device)
    img_orig = G(latent_code_init, label, truncation_psi=args.truncation_psi, noise_mode=args.noise_mode)
